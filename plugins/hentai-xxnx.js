...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  if (!chat.nsfw)
    throw `💝 TIKA HENTAI 💝  does not support NSFW.\n\nTo turn it on, use: ${usedPrefix}on nsfw \n\n🤤🤤🤤🤤🤤🤤🤤🤤🤤`;
  ...
  if (!text)
    throw `💝 TIKA HENTAI 💝 , What do you want to search on 💝 TIKA HENTAI 💝?\n🕸️ Usage: ${usedPrefix + command} <search>\n\nEx:Mia khalifa🤣  or bza can use a link \nEx: .xnxx link *`;

  m.react(rwait);

  if (text.includes('http://') || text.includes('https://')) {
    if (!text.includes('xnxx.com'))
      return m.reply(`Invalid link. Only xnxx.com, Generated 💝 TIKA HENTAI 💝`);
...
🕸️ QUEEN-HENTAI
    ...
    try {
      ...
      if (searchJson.data && searchJson.data.length > 0) {
        ...
        m.reply(` No results found for the 💝 TIKA HENTAI 💝 search list.`);
      }
    } catch (e) {
